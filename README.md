...
and a market-scope signal. The market-scope call returns 3 confidence levels —
`directional` (LLM fallback), `evidence-backed` (corpus + ≥1 web source), and
`quantitative` (deterministic corpus rules + fully populated stats) — surfaced
as a small badge next to the existing color-coded direction chip. Retrieval is
benchmarked against a labeled 300-idea benchmark drawn from the public YC +
Product Hunt + HN corpus — see [`docs/METHODOLOGY.md`](docs/METHODOLOGY.md)
for the metric definitions and the
...
signal` + `assemble_verdict`. The market-scope call is now a 4-step pipeline
(Phase 4.1–4.4): corpus density from `companies` + `company_embeddings` →
SearXNG-augmented web layer for sparse directions → Claude Sonnet 4.5
synthesis over the corpus stats + web snippets → envelope assembly into
`market_scope_signal` (direction + confidence tier + evidence trail + optional
`MarketScopeQuant`). The frontend renders the 3-level confidence badge
(`directional` / `evidence-backed` / `quantitative`) next to the color-coded
direction chip. The Temporal UI on `http://localhost:8233` shows per-idea
runs, activity history, retry events, and signal channels.
...
1. **Market-scope is corpus-grounded, not a SEMrush replacement.** We
   compute density / recency / category distribution from our 11K-company
   corpus and add a SearXNG-augmented evidence layer (via Firecrawl) for
   sparse directions. The API returns a 3-level `confidence` field —
   `directional` (the old stub behavior, when the corpus is thin),
   `evidence_backed` (corpus + ≥1 web source), `quantitative` (corpus stats
   + 4-direction envelope fully populated) — surfaced in the frontend as
   a badge next to the color-coded direction chip (`directional` is gray,
   `evidence-backed` is blue with a tooltip naming the web sources,
   `quantitative` is green with a tooltip naming the corpus stats). The
   implementation is described in [`docs/PHASE-4.md`](docs/PHASE-4.md); the
   eval-harness regression enforces a hard-coded accuracy floor (typed
   constant, not config). We do not estimate search volume or TAM — for
   that, use SEMrush.
