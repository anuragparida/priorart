...
1. **Market-scope is corpus-grounded, not a SEMrush replacement.** We
   ...
   + 4-direction envelope fully populated). The implementation is described
   in [`docs/PHASE-4.md`](docs/PHASE-4.md); the eval-harness regression
   enforces a hard-coded accuracy floor (typed constant, not config). We do
   not estimate search volume or TAM — for that, use SEMrush.
